...
const CLASS_DAYS: Day[] = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];
const FULL_WEEK_DAYS: Day[] = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];

const COLORS = [
  'bg-emerald-500', 'bg-blue-500', 'bg-indigo-500', 'bg-purple-500', 
  'bg-rose-500', 'bg-orange-500', 'bg-amber-500', 'bg-slate-500'
];
...
export default function App() {
  ...
  const activeEvent = useMemo(() => {
    ...
    if (activeClass) return { id: activeClass.id, title: activeClass.subject, endTime: activeClass.endTime, type: 'Class', color: activeClass.color };
    ...
    if (activePlanner) return { id: activePlanner.id, title: activePlanner.title, endTime: activePlanner.endTime, type: activePlanner.category, color: activePlanner.color };
  ...
  const nextEvent = useMemo(() => {
    const allToday = [
      ...classes.map(c => ({ id: c.id, title: c.subject, startTime: c.startTime, type: 'Class' })),
      ...plannerEvents.map(p => ({ id: p.id, title: p.title, startTime: p.startTime, type: p.category }))
    ].filter(e => {
        // If we have an active event, we want the next one
        if (activeEvent && e.id === activeEvent.id) return false;
        return timeToMinutes(e.startTime) >= currentMinutes;
    });

    return allToday.sort((a, b) => timeToMinutes(a.startTime) - timeToMinutes(b.startTime))[0] || null;
  }, [classes, plannerEvents, currentDay, currentMinutes, activeEvent]);
  ...
  useEffect(() => {
    if (prevActiveId.current !== null && activeEvent?.id !== prevActiveId.current) {
  ...
  const [newClass, setNewClass] = useState<Omit<ClassSession, 'id' | 'day'> & { days: Day[] }>({
    subject: '',
    days: ['Monday'],
    startTime: '09:00',
    endTime: '10:30',
  ...
  const [newPlannerEvent, setNewPlannerEvent] = useState<Omit<PlannerEvent, 'id' | 'day'> & { days: Day[] }>({
    title: '',
    days: ['Monday'],
    startTime: '08:00',
    endTime: '09:00',
  ...
  const handleSaveClass = (e: FormEvent) => {
    e.preventDefault();
    if (editingId) {
      setClasses(classes.map(c => c.id === editingId ? { ...newClass, day: newClass.days[0], id: editingId } : c));
    } else {
  ...
  const handleSavePlannerEvent = (e: FormEvent) => {
    e.preventDefault();
    if (editingId) {
      setPlannerEvents(plannerEvents.map(p => p.id === editingId ? { ...newPlannerEvent, day: newPlannerEvent.days[0], id: editingId } : p));
    } else {
  ...
  const hoursClasses = Array.from({ length: endHourClasses - startHourClasses + 1 }, (_, i) => startHourClasses + i);

  const startHourPlanner = 6; // 6 AM
  const endHourPlanner = 24;  // Midnight
  const hoursPlanner = Array.from({ length: endHourPlanner - startHourPlanner + 1 }, (_, i) => startHourPlanner + i);
              ...
              >
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <h2 className="text-3xl font-black mb-2">Class Timetable</h2>
                    <p className="text-neutral-500 max-w-xs mb-8">View and manage your academic schedule across the week.</p>
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-neutral-500 max-w-xs mb-8">Detailed weekly overview covering study, social, and personal goals.</p>
                ...
                <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-6">
                  <div className={`col-span-1 sm:col-span-2 lg:col-span-1 p-8 rounded-[2.5rem] border transition-all flex flex-col justify-between relative overflow-hidden ${activeEvent ? `${activeEvent.color} text-white border-transparent shadow-xl` : 'bg-white border-neutral-100 shadow-sm'}`}>
                    {activeEvent && (
                      <motion.div 
                    ...
                    <div className="relative z-10">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${activeEvent ? 'bg-white/20' : 'bg-amber-50 text-amber-600'}`}>
                        <Target size={20} />
                      </div>
                      <p className={`text-xs font-bold uppercase tracking-widest ${activeEvent ? 'text-white/60' : 'text-neutral-400'}`}>
                        {activeEvent ? `Current: ${activeEvent.type}` : 'No Active Session'}
                      </p>
                      
                      {activeEvent ? (
                        <div className="mt-2">
                          <h3 className="text-2xl font-black truncate">{activeEvent.title}</h3>
                          <div className="mt-4 flex flex-col">
                            <span className="text-4xl font-black tracking-tighter tabular-nums">
                              {timeLeft?.formatted}
                            </span>
                            <span className="text-[10px] uppercase font-black tracking-widest mt-1 opacity-70">Remaining</span>
                          </div>
                        </div>
                      ) : (
                  ...
                  <div className="bg-white p-6 rounded-3xl border border-neutral-100 shadow-sm">
                     ...
                     <p className="text-neutral-400 text-xs font-bold uppercase tracking-widest">Productivity</p>
                     <p className="text-2xl font-black mt-1">High</p>
                  </div>
                  <div className="bg-white p-6 rounded-3xl border border-neutral-100 shadow-sm">
                     <div className="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center mb-4 cursor-pointer hover:bg-indigo-100 transition-all" onClick={requestNotificationPermission}>
                        <Info size={20} />
                     </div>
                     <p className="text-neutral-400 text-xs font-bold uppercase tracking-widest">Reminders</p>
                     <p className="text-xl font-black mt-1">
                        {Notification.permission === 'granted' ? "Enabled" : "Disabled"}
                     </p>
                     {Notification.permission !== 'granted' && (
                       <button onClick={requestNotificationPermission} className="text-[10px] font-black text-indigo-600 uppercase mt-2 hover:underline">Enable Alerts</button>
                     )}
                  </div>
                  <div className="bg-white p-6 rounded-3xl border border-neutral-100 shadow-sm">
                     ...
                     <p className="text-neutral-400 text-xs font-bold uppercase tracking-widest">Week Focus</p>
              ...
              >
                <div className="overflow-x-auto print:overflow-visible">
                  <div className="min-w-[1000px] grid grid-cols-[80px_repeat(5,1fr)] print:min-w-0">
                    ...
                    {CLASS_DAYS.map((day) => (
                      <div key={day} className="py-5 text-center border-b border-neutral-100 font-black text-xs uppercase tracking-widest bg-neutral-50/50 text-neutral-400">
                        {day}
                      </div>
                    ))}
                    ...
                    <div className="border-r border-neutral-100 relative h-[840px] bg-neutral-50/20">
                      {hoursClasses.map((hour) => (
                        <div 
                          ...
                          style={{ top: `${(hour - startHourClasses) * 60 + 10}px` }}
                        >
                          {hour}:00
                        </div>
                      ))}
                    ...
                    {CLASS_DAYS.map((day) => (
                      <div key={day} className="relative h-[840px] border-r last:border-r-0 border-neutral-100 bg-grid-pattern">
                        {hoursClasses.map((hour) => (
                          <div 
                            key={hour} 
                            className="absolute w-full h-[1px] bg-neutral-50"
                            style={{ top: `${(hour - startHourClasses) * 60}px` }}
                          />
                        ))}
                        ...
                        <AnimatePresence>
                          {classes
                            ...
                            .map((c) => {
                              ...
                              const top = minutesToPosition(startMin, startHourClasses);
                              ...
                              return (
                                ...
                                >
                                  <div className="flex justify-between items-start">
                                    <h3 className="font-bold text-xs leading-tight pr-4 drop-shadow-sm">{c.subject}</h3>
                ...
                <div className="overflow-x-auto print:overflow-visible">
                  <div className="min-w-[1400px] grid grid-cols-[80px_repeat(7,1fr)] print:min-w-0">
                    ...
                    {FULL_WEEK_DAYS.map((day) => (
                      <div key={day} className="py-5 text-center border-b border-neutral-100 font-black text-[10px] uppercase tracking-widest bg-neutral-50/50 text-neutral-400">
                        {day}
                      </div>
                    ))}
                    ...
                    <div className="border-r border-neutral-100 relative h-[1080px] bg-neutral-50/20">
                      {hoursPlanner.map((hour) => (
                        <div 
                          key={hour} 
                          className="absolute w-full text-right pr-4 font-mono text-[10px] text-neutral-300 font-bold"
                          style={{ top: `${(hour - startHourPlanner) * 60 + 10}px` }}
                        >
                          {hour === 24 ? '00' : hour}:00
                        </div>
                      ))}
                    ...
                    {FULL_WEEK_DAYS.map((day) => (
                      <div key={day} className="relative h-[1080px] border-r last:border-r-0 border-neutral-100 bg-grid-pattern">
                        {hoursPlanner.map((hour) => (
                          <div 
                            key={hour} 
                            className="absolute w-full h-[1px] bg-neutral-50"
                            style={{ top: `${(hour - startHourPlanner) * 60}px` }}
                          />
                        ))}
                        ...
                        <AnimatePresence>
                          {plannerEvents
                            ...
                            .map((p) => {
                              ...
                              const top = minutesToPosition(startMin, startHourPlanner);
                              const height = endMin - startMin;
                                ...
                                >
                                  <div className="flex justify-between items-start">
                                    <div className="flex flex-col">
                                      <span className="text-[8px] font-black uppercase opacity-60 leading-none mb-1 tracking-tighter truncate">{p.category}</span>
                                      <h3 className="font-bold text-[11px] leading-tight truncate">{p.title}</h3>
                                    </div>
                                    <button 
                                      onClick={(e) => { e.stopPropagation(); removePlannerEvent(p.id); }}
      ...
      <AnimatePresence>
        {isAddingMode && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={resetForm} className="absolute inset-0 bg-neutral-900/60 backdrop-blur-sm" />
            <motion.div initial={{ opacity: 0, y: 50, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 50, scale: 0.95 }} className="relative w-full max-w-xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden">
               <div className="px-8 py-6 border-b border-neutral-100 flex justify-between items-center bg-white">
                  <h2 className="text-2xl font-black">
                     {view === 'classes' ? (editingId ? 'Edit Class' : 'Add Class') : (editingId ? 'Edit Event' : 'New Plan')}
                  </h2>
                  <button onClick={resetForm} className="p-3 hover:bg-neutral-100 rounded-full transition-colors text-neutral-500">
                     <X size={24} />
                  </button>
               </div>

               <form onSubmit={view === 'classes' ? handleSaveClass : handleSavePlannerEvent} className="p-8 space-y-6">
                {view === 'classes' ? (
                  <div className="space-y-4">
                    <div>
                      <label className="block text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-2">Subject Name</label>
                      <input required type="text" className="w-full bg-neutral-50 px-6 py-4 rounded-2xl border border-neutral-100 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 transition-all outline-none font-bold" value={newClass.subject} onChange={(e) => setNewClass({ ...newClass, subject: e.target.value })} />
                    </div>
                    <div>
                        <label className="block text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-2">Days {!editingId && "(Select multiple to repeat)"}</label>
                              ...
                              onClick={() => {
                                if (editingId) {
                                  ...
                                  const days = newClass.days.includes(day) 
                                    ...
                                    : [...newClass.days, day];
                                  if (days.length > 0) setNewClass({ ...newClass, days });
                                }
                              }}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-2">Theme</label>
                        <div className="flex flex-wrap gap-2 pt-2">
                          {COLORS.map(color => (
                            <button key={color} type="button" onClick={() => setNewClass({ ...newClass, color })} className={`w-6 h-6 rounded-full ${color} transition-transform ${newClass.color === color ? 'scale-125 ring-2 ring-offset-2 ring-indigo-500' : 'hover:scale-110'}`} />
                          ))}
                        </div>
                      </div>
                      <div className="grid grid-rows-2 gap-2">
                        <input required type="time" className="bg-neutral-50 px-6 py-4 rounded-2xl border border-neutral-100 outline-none font-bold text-sm" value={newClass.startTime} onChange={(e) => setNewClass({ ...newClass, startTime: e.target.value })} />
                        <input required type="time" className="bg-neutral-50 px-6 py-4 rounded-2xl border border-neutral-100 outline-none font-bold text-sm" value={newClass.endTime} onChange={(e) => setNewClass({ ...newClass, endTime: e.target.value })} />
                      </div>
                    </div>
                    <input type="text" placeholder="Location (Optional)" className="w-full bg-neutral-50 px-6 py-4 rounded-2xl border border-neutral-100 outline-none font-bold" value={newClass.location} onChange={(e) => setNewClass({ ...newClass, location: e.target.value })} />
                  </div>
                ) : (
                  <div className="space-y-4">
                    <div>
                      <label className="block text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-2">Event Title</label>
                      <input required type="text" className="w-full bg-neutral-50 px-6 py-4 rounded-2xl border border-neutral-100 focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-50 transition-all outline-none font-bold" value={newPlannerEvent.title} onChange={(e) => setNewPlannerEvent({ ...newPlannerEvent, title: e.target.value })} />
                    </div>
                    <div>
                        <label className="block text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-2">Days {!editingId && "(Select multiple to repeat)"}</label>
                        <div className="flex flex-wrap gap-2">
                          {FULL_WEEK_DAYS.map(day => (
                            <button
                              ...
                              onClick={() => {
                                if (editingId) {
                                  setNewPlannerEvent({ ...newPlannerEvent, days: [day] });
                                } else {
                                  const days = newPlannerEvent.days.includes(day) 
                                    ? newPlannerEvent.days.filter(d => d !== day) 
                                    : [...newPlannerEvent.days, day];
                                  if (days.length > 0) setNewPlannerEvent({ ...newPlannerEvent, days });
                                }
                              }}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <select 
                          ...
                          onChange={(e) => {
                            ...
                            setNewPlannerEvent({ ...newPlannerEvent, category: cat, color: CATEGORIES[cat] });
                          }}
                        >
                      ...
                      <div className="grid grid-rows-2 gap-2">
                        <input required type="time" className="bg-neutral-50 px-6 py-4 rounded-2xl border border-neutral-100 outline-none font-bold text-sm" value={newPlannerEvent.startTime} onChange={(e) => setNewPlannerEvent({ ...newPlannerEvent, startTime: e.target.value })} />
                        <input required type="time" className="bg-neutral-50 px-6 py-4 rounded-2xl border border-neutral-100 outline-none font-bold text-sm" value={newPlannerEvent.endTime} onChange={(e) => setNewPlannerEvent({ ...newPlannerEvent, endTime: e.target.value })} />
                      </div>
                    </div>
                  </div>
                )}
                
                <button type="submit" className={`w-full ${view === 'classes' ? 'bg-indigo-600' : 'bg-emerald-600'} text-white font-black py-5 rounded-2xl transition-all shadow-xl active:scale-95 text-lg mt-4`}>
